...
build_language_model(void)
{
    ...
    // NOTE(sammynilla): Common standard lib stuff
    sm_key("NamespaceSTD", "std"); // NOTE(sammynilla): Potentially temporary.
    sm_key("AutoPtr", "auto_ptr"); // Deprecated
    sm_key("WeakPtr", "weak_ptr");
    sm_key("SharedPtr", "shared_ptr");
    sm_key("UniquePtr", "unique_ptr");
    // NOTE(sammynilla): Adding new cpp keywords
    sm_key("Auto");
    sm_key("ConstExpr");
    
    // HACK(sammynilla): BATCH KEYWORDS (in place until we find an alternative).
    sm_key("BAT_ASSOC", "assoc");
    sm_key("BAT_CLS", "cls");
    //sm_key("BAT_CMD", "cmd"); // Causing issues with project.4coder commands
    sm_key("BAT_COMP", "comp");
    sm_key("BAT_CONVERT", "convert");
    //sm_key("BAT_COPY", "copy"); // Also causing issues for some reason
    sm_key("BAT_DATE", "date");
    sm_key("BAT_DEL", "del");
    sm_key("BAT_DIR", "dir");
    sm_key("BAT_DISKPART", "diskpart");
    sm_key("BAT_DRIVERQUERY", "driverquery");
    sm_key("BAT_ECHO", "echo");
    sm_key("BAT_EXIT", "exit");
    sm_key("BAT_EXPAND", "expand");
    sm_key("BAT_FC", "fc");
    sm_key("BAT_FIND", "find");
    sm_key("BAT_FORMAT", "format");
    sm_key("BAT_HELP", "help");
    sm_key("BAT_IPCONFIG", "ipconfig");
    sm_key("BAT_LABEL", "label");
    sm_key("BAT_MD", "md");
    sm_key("BAT_MORE", "more");
    sm_key("BAT_MOVE", "move");
    sm_key("BAT_NET", "net");
    sm_key("BAT_PATH", "path");
    sm_key("BAT_PAUSE", "pause");
    sm_key("BAT_PING", "ping");
    sm_key("BAT_RD", "rd");
    sm_key("BAT_REM", "rem");
    sm_key("BAT_REN", "ren");
    sm_key("BAT_SET", "set");
    sm_key("BAT_SHUTDOWN", "shutdown");
    sm_key("BAT_SORT", "sort");
    sm_key("BAT_START", "start");
    sm_key("BAT_SYSTEMINFO", "systeminfo");
    sm_key("BAT_TASKKILL", "taskkill");
    sm_key("BAT_TASKLIST", "tasklist");
    sm_key("BAT_TIME", "time");
    sm_key("BAT_TITLE", "title");
    sm_key("BAT_TREE", "tree");
    sm_key("BAT_TYPE", "type");
    sm_key("BAT_VER", "ver");
    sm_key("BAT_VOL", "vol");
    sm_key("BAT_XCOPY", "xcopy");
    // HACK(sammynilla): BATCH OPERATOR KEYWORDS
    sm_key("BAT_EQU", "equ");
    sm_key("BAT_NEQ", "neq");
    sm_key("BAT_LSS", "lss");
    sm_key("BAT_LEQ", "leq");
    ...
    sm_select_base_kind(TokenBaseKind_Keyword);
    // NOTE(sammynilla):
    // THE MAJORITY OF THESE SHOULD BE UNNECESSARY AND HANDLED IN LANGUAGE SPECIFIC HIGHLIGHTING.s
    sm_key("PPDefined", "defined");
    sm_key("PPHasInclude", "__has_include");
    sm_key("PPHasCppAttribute", "__has_cpp_attribute");
    // NOTE(sammynilla): i.e. #pragma once
    sm_key("PPOnce", "once");
    // NOTE(sammynilla): i.e. #pragma warning(disable:xxxx)
    sm_key("PPDisable", "disable");
    sm_key("PPEnable", "enable");
    // NOTE(sammynilla): i.e. #pragma warning(push), #pragma warning(pop)
    sm_key("PPPush", "push");
    sm_key("PPPop", "pop");
