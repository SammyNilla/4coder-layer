 ...
 function void
Fleury4RenderBraceLines(Application_Links * app,
                        Buffer_ID buffer, View_ID view, Text_Layout_ID text_layout_id,
                        i64 pos)
{
    ...
    float x_position =
        view_get_screen_rect(app, view).x0 + 4 -
        view_get_buffer_scroll(app, view).position.pixel_shift.x;
    
    // NOTE(sammynilla):
    // Adjust for the event that line number margins are in place.
    // This is due to the fact that Colby's line number margin code adds extra padding.
    if (global_config.show_line_number_margins)
    {
        i64 line_count = buffer_get_line_count(app, buffer);
        i64 line_count_digit_count = digit_count_from_integer(line_count, 10) + 2;
        x_position += (line_count_digit_count * metrics.space_advance);
    }
    
    u32 indent_advance = 4; // NOTE(sammynilla):  default indent spacing
    if (global_config.enable_virtual_whitespace)
        indent_advance = global_config.virtual_whitespace_regular_indent;
    ...
    for (i32 i = ranges.count - 1; i >= 0; i -= 1)
    {
        ...
        x_position += metrics.space_advance * indent_advance;
    }
}
